...
async function importData() {
  try {
    ...
    const $Promise = [];
    $Promise.push(
      axios.get('https://custom-api-omega.vercel.app/api/foods')
    );
    $Promise.push(
      axios.get('https://custom-api-omega.vercel.app/api/chefs')
    );


    $Promise.push(
      axios.get('https://custom-api-omega.vercel.app/api/blogs')
    );


    const [foodsResponse, chefsResponse , blogsResponse] = await Promise.all($Promise);
    ...
    for (const blog of blogs) {
      ...
      let imageRef = null;
      if (blog.image) {
        imageRef = await uploadImageToSanity(blog.image);
      }
      ...
      console.log('Uploading food to Sanity:', sanityBlog.name);
      const result = await client.create(sanityBlog);
      console.log(`Blog uploaded successfully: ${result._id}`);
    }
